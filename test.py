...
def func(x, y, t):
    ...
    r3 = np.sqrt((x-3)**2 + y**2)   
    r4 = np.sqrt((x-2)**2 + y**2)    
    r5 = np.sqrt((x-1)**2 + y**2)
    r6 = np.sqrt((x)**2 + y**2)    
    r7 = np.sqrt((x+1)**2 + y**2)
    r8 = np.sqrt((x+2)**2 + y**2)
    r9 = np.sqrt((x+3)**2 + y**2)
    r10 = np.sqrt((x+4)**2 + y**2)
    r11 = np.sqrt((x+5)**2 + y**2)
    r12 = np.sqrt((x+6)**2 + y**2)
    """
    for i in range(14):
        r = np.sqrt((x+i-6)**2 + (y-6)**2)
        if(i == 0):
            z = np.exp(j*k*r)/k*3
        else:
            temp = z+np.exp(j*k*r)/k*3
            z = temp
